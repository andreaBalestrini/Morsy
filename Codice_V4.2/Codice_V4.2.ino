...
void loop() {
  ...
  State GameState = digitalRead(buttonGame) ? UP : DOWN;
  
  if (DotState == DOWN || LineState == DOWN || SpaceState == DOWN || GameState == DOWN || EndCharState == DOWN) lastPress = millis();;
  
  //if (DotState == DOWN || LineState == DOWN) { // leggo i punti e linee
  if (buttonReader(buttonDot, nextRead) || buttonReader(buttonLine, nextRead)) {
    //lastPress = millis();
    //if (nextRead) {
      nextRead = false;
  ...
  else if (EndCharState == DOWN) { //leggo l'invio
    //lastPress = millis();
    if (nextRead) {
      nextRead = false;
  ...
  if (DotState == UP && LineState == UP && SpaceState == UP && GameState == UP && EndCharState == UP && !nextRead && ((millis() - lastPress) > maxinputtime )) nextRead = true; delay(10);
}
