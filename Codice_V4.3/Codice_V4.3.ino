...
/*Bug
  Molta memoria occupata, se possibile ridurre le linee di codice
*/
...
const char powered[]="Hackability@Polito";
...
void saluto() {
  ...
  delay(3000);
  lcd.clear();
  if(!timeTrigger(analogRead(timePin))){
    lcd.setCursor(0, 0);
...
bool timeTrigger(int reg) {
  ...
  lcd.print("Esperto");
  Serial.println(reg);
  if (reg+1 > 0) { //seleziono la velocità, dall'alto: Lento, Normale, Esperto, Pro
    if (reg <= 320) {
    ...
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Time Reg Error");
    lcd.setCursor(0, 1);
    lcd.print("Riavviare il");
    lcd.setCursor(0, 2);
    lcd.print("dispositivo.");
    BTserial.print("Time Reg Error\r\nRiavviare il dispositivo\r\n");
    pro = true;
  }
  delay(5000);
  if(!pro){
